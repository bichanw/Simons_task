...
starting_d_contrast = 0.1; % starting delta contrast
...
Params = [Params, rand(NTrials,1)<1]; % random validity
...
% set up staircase
Rev = 0;
StepSize = (starting_d_contrast)/3;
resp_rw  = [];
Xnext    = starting_d_contrast; 
P_reds   = [];
d_contrast = starting_d_contrast;
...
for iTrial = 1:NTrials
	...
	sqInfo.c_tar = round(sqInfo.c * (1-d_contrast));
	if Params(iTrial,4)
	...
	Resps = [Resps;resp,rt,d_contrast];

	% staircase
	[d_contrast,Threshold,Rev,StepSize] = StairCase(Resps(:,3),Resps(:,1)==Params(1:iTrial,4),2,Rev,StepSize);
	d_contrast = max([d_contrast 0]);
...
T_out = array2table([Resps,Time],'VariableNames',{'resp','rt','d_contrast','t_fix','t_sq_on','t_dot_on','t_dot_off','t_tar_on','t_tar_off'});
Params = array2table(Params,'VariableNames',{'P_reds','dot_off_2_tar_on','validity','tar_lr'});
save(['Results/' subjectID '_rhythm_thresh.mat'],'T_out','Params','Threshold');
